...
@interface QWInterchangeView_moveItem()
...
@implementation QWInterchangeView_moveItem
...
-(void)setM_canItercanged:(_Bool)aM_canItercanged
{
    if (aM_canItercanged) {//互换模式
        [QWAnimation SetRockerKeyAnimation:self 
                                   delegate:self 
                                        key:@"Interchange" 
                                   duration:ROCKER_DURATION 
                                      angle:ROCKER_ANGLE 
                                  direction:QWAnimationRockDirectionLeft];
    }
    else
      {
