...
#import "QWRockerEvent.h"


@interface QWRockerEvent()

-(void)checkMoveInRect:(CGPoint )point;
-(void)sendDirection:(QWRockerEventDirection)aDirection;
-(bool)checkInMinCircle:(CGPoint)point;
-(CGPoint)checkInMaxCircle:(CGPoint)point;
@end

@implementation QWRockerEvent
...
{
    [self sendDirection:QWRockerEventNone];
...
-(void)sendDirection:(QWRockerEventDirection)aDirection
{
    if (delegate && [delegate respondsToSelector:@selector(QWRockerEventClicked: direction:)]) {
        [delegate QWRockerEventClicked:self direction:aDirection];
    }
}
