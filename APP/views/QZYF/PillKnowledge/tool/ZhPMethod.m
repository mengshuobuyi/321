...
@end
BOOL isSpecialstring(NSString *text){
...
CGSize getTempTextSize(NSString* text, UIFont* font, CGFloat width)
{
...
- (id)init
{
    if (self = [super init]) {
        ...
        blackView.layer.backgroundColor = RGBHex(qwColor8).CGColor;//(42,42,42);
        blackView.layer.cornerRadius = 3;
        blackView.alpha = 0.8;
        showShadow(blackView, CGSizeMake(1, 1));
        [bgview addSubview:blackView];
        
        label = addLabelObj(bgview, @[@(NOTICE_BOX_TAG+1), RECT_OBJ(0, 0, 0, 0), RGBHex(qwColor4), fontSystem(kFontS5)]);
    }
    return  self;
}
...
- (void)show
{
    ...
    CGFloat h = getTempTextSize(list[0], fontSystem(kFontS5), MAX_WIDTH).height;
