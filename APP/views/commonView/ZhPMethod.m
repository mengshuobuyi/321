...
BOOL isTelePhoneNumber(NSString *text){
    NSString *phoneRegex = @"^\\(?\\d{3,5}[-\\)]?\\d{6,8}$";
...
- (id)init
{
    if (self = [super init]) {
        ...
        blackView.layer.backgroundColor = RGB(42, 42, 42).CGColor;
        blackView.layer.cornerRadius = 3;
        blackView.alpha = 0.8;
        showShadow(blackView, CGSizeMake(1, 1));
        [bgview addSubview:blackView];
        
        label = addLabelObj(bgview, @[@(NOTICE_BOX_TAG+1), RECT_OBJ(0, 0, 0, 0), RGB(255, 255, 255), fontSystem(kFontSize12)]);
    }
    return  self;
}
...
- (void)show
{
    ...
    CGFloat h = getTextSize(list[0], fontSystem(kFontSize12), MAX_WIDTH).height;
