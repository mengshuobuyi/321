...
const CGFloat kEmojiWidthRatioWithLineHeight = 1.25;//和字体高度的比例  1.25

const CGFloat kEmojiOriginYOffsetRatioWithLineHeight = 0.10; //表情绘制的y坐标矫正值，和字体高度的比例，越大越往下 0.10
...
//    [self initPlist];
    ...
    UIColor *commonLinkColor = RGBHex(qwColor5);//[UIColor colorWithRed:0.112 green:0.000 blue:0.791 alpha:1.000];
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            if (imageName) {
                ...
                if([imageName isEqualToString:@"info_ic_expert"] || [imageName isEqualToString:@"info_pharmacist"]) {
                    runBounds.size.width = 12;
                    runBounds.size.height = 17;
                }else if ([imageName isEqualToString:@"post_ic_lind"]) {
                    runBounds.size.width = 13;
                    runBounds.size.height = 13;
                    runBounds.origin.y += 5.5;
                    runBounds.origin.x += 7.5;
                }
                  ...
                  WithFont:(UIFont *)font
{
    ...
    [displayTextView setEmojiText:emojiText];
    displayTextView.font = font;
    displayTextView.lineBreakMode = NSLineBreakByWordWrapping;
    displayTextView.isNeedAtAndPoundSign = YES;
    displayTextView.minimumLineHeight = 25;
    //displayTextView.frame = CGRectMake(0, 0, size.width, size.height);
    CGSize retSzie = [displayTextView sizeThatFits:size];
    retSzie.height = MAX(25, retSzie.height);
    return retSzie;
    // return CGSizeMake(displayTextView.frame.size.width, displayTextView.frame.size.height);
}
...
+ (CGSize)expertInfoCommentNeedSizeWithText:(NSString *)emojiText WithConstrainSize:(CGSize)size
{
    ...
    displayTextView.lineBreakMode = NSLineBreakByWordWrapping;
    displayTextView.isNeedAtAndPoundSign = YES;
    displayTextView.minimumLineHeight = 14;
    CGSize retSzie = [displayTextView sizeThatFits:size];
    retSzie.height = MAX(14, retSzie.height);
    return retSzie;
    // return CGSizeMake(displayTextView.frame.size.width, displayTextView.frame.size.height);
}
...
didSelectLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
{
    ...
    }else if (result.resultType == NSTextCheckingTypeLink)
    {
        if(self.emojiDelegate && [self.emojiDelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)])
        {
            NSString *string = [self.emojiText substringWithRange:result.range];
            for(NSDictionary *dict in self.tagList)
            {
                NSString *subString = [self.emojiText substringWithRange:NSMakeRange([dict[@"start"] integerValue], [dict[@"length"] integerValue])];
                if([string isEqualToString:subString])
                {
                    if([dict[@"tagType"] integerValue] == 1)
                    {
                        //表示药品
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:dict[@"tagId"] withType:MLEmojiLabelLinkTypeMedicineDetail];
                    }else if ([dict[@"tagType"] isEqualToString:@"outerLink"]) {
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:dict[@"tagId"] withType:MLEmojiLabelLinkTypeOuterLink];
                    }else if ([dict[@"tagType"] isEqualToString:@"postDetail"]) {
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:dict[@"tagId"] withType:MLEmojiLabelLinkTypePostDetail];
                    }else{
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:dict[@"tagId"] withType:MLEmojiLabelLinkTypeDrugGuide];
                    }
                    break;
                }
            }
...
- (void)addLinkTags:(NSArray *)tagList
{
    ...
    for(NSDictionary *tag in tagList)
    {
        ...
        [self addLinkToURL:[NSURL URLWithString:@""] withRange:NSMakeRange(start, length)];
    }
}
