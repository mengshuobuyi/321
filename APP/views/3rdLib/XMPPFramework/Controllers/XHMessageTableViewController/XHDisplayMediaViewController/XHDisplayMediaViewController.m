...
- (void)setMessage:(id<XHMessageModel>)message {
    _message = message;
    if ([message messageMediaType] == XHBubbleMessageMediaTypeLocation) {
        ...
        //            [self.photoImageView setImageWithURL:[NSURL URLWithString:[message thumbnailUrl]] placeholderImage:[UIImage imageNamed:@"placeholderImage"]];
        //        }
//        NSLog(@"------message.photp----%@",NSStringFromCGSize(message.photo.size));
        if ( !message.photo) {
            [self.photoImageView setImageWithURL:[NSURL URLWithString:[message thumbnailUrl]] placeholderImage:[UIImage imageNamed:@"placeholderImage"]];
        }
    }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if ([self.message messageMediaType] == XHBubbleMessageMediaTypeLocation) {
