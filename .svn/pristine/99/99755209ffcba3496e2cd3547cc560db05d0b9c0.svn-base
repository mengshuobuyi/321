...
- (void)setUpData:(id)data
{
    ...
    }else
    {
        ...
        if (model.userType == 3 || model.userType == 4)
        {
            ...
            float maxWidth;
            if (model.userType == 3) {
                maxWidth = APP_W-85-nameSize.width-13-105;
            }else if (model.userType == 4){
                maxWidth = APP_W-85-nameSize.width-13-115;
            }
        ...
        {
            //专家
            if (model.id && QWGLOBALMANAGER.configure.expertPassportId && [model.id isEqualToString:QWGLOBALMANAGER.configure.expertPassportId])
            {
