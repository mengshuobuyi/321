...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    if(activeTabIndex == self.activeTabIndex)
    {
        TabView *activeTabView = [self tabViewAtIndex:self.activeTabIndex];
        [(UILabel*)activeTabView.labelView setTextColor:_indicatorColor];
        return;
    }
    
    __block TabView *activeTabViewOld;
    __block TabView *activeTabViewNew;
    
    // Set to-be-inactive tab unselected
    activeTabViewOld = [self tabViewAtIndex:self.activeTabIndex];
    
    // Set to-be-active tab selected
    activeTabViewNew = [self tabViewAtIndex:activeTabIndex];
    
    CGPoint newpoint = CGPointMake(activeTabViewNew.center.x, self.bottomView.center .y);
    
    [UIView animateWithDuration:0.2f animations:^{
        
        self.bottomView.center = newpoint;
        self.bottomView.bounds = CGRectMake(0, 0, activeTabViewNew.bounds.size.width, self.bottomView.bounds.size.height);
        ...
        if (finished) {
            ...
            for (int i = 0; i < _tabCount; i++) {
                ...
                if (activeTabIndex == i) {
                    [(UILabel*)tabView.labelView setTextColor:_indicatorColor];
                }
                else
                {
                    [(UILabel*)tabView.labelView setTextColor:[UIColor blackColor]];
                }
...
- (void)reloadData {
    ...
    if (self.startFromSecondTab) {
        ...
        TabView *activeTabView = [self tabViewAtIndex:0];
        TabView *activeTabView2 = [self tabViewAtIndex:1];
        self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(activeTabView.frame.size.width, self.tabHeight-4.0f, activeTabView2.frame.size.width, 2.0f)];
        [self.view addSubview:self.bottomView];
        [self.bottomView setBackgroundColor:_indicatorColor];
        
    } else {
        viewController = [self viewControllerAtIndex:0];
        TabView *activeTabView = [self tabViewAtIndex:0];
        self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabHeight-4.0f, activeTabView.frame.size.width, 2.0f)];
        [self.view addSubview:self.bottomView];
        [self.bottomView setBackgroundColor:_indicatorColor];
    }
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        NSString* aString = [self.dataSource viewPager:self viewForTabAtIndex:index];
        
        NSDictionary *attribute = @{NSFontAttributeName: [UIFont systemFontOfSize:15]};
        
        CGSize titleSize = [aString boundingRectWithSize:CGSizeMake(100, 0) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
        ...
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, titleSize.width+21.9f, self.tabHeight)];
        [tabView addSubview:label];
        tabView.labelView = label;
        [tabView setClipsToBounds:YES];
        [tabView setIndicatorColor:self.indicatorColor];
        [tabView setBackgroundColor:[UIColor clearColor]];
        
        label.center = tabView.center;
