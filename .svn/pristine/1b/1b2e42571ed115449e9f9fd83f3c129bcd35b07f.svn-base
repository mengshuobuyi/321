...
/*
     ...
     h5/imConsult/race       专家抢答
     api/consult/qReply      获取常用话术列表
 */
...
BOOL const ExpertXPallowsSendFace = YES;
BOOL const ExpertXPallowsSendVoice = NO;
BOOL const ExpertXPallowsSendMultiMedia = YES;
BOOL const ExpertXPallowsPanToDismissKeyboard = NO;//是否允许手势关闭键盘，默认是允许
const int ExpertXPalertResendIdentifier = 10000;
const int ExpertXPalertDeleteIdentifier = 10001;
...
@implementation ExpertXPChatViewController
...
//    self.interlocutiondetailModel.unreadCounts = @"0";

    [InterlocutionDetailListModel updateObjToDB:self.interlocutiondetailModel WithKey:[NSString stringWithFormat:@"%@",self.interlocutiondetailModel.consultId]];
...
- (void)viewDidLoad {
    ...
    //点对点变群聊的，找得以前的代码---cj
    if(self.interlocutiondetailModel && self.interlocutiondetailModel.name && ![self.interlocutiondetailModel.name isEqualToString:@""])
    {
        self.title = self.interlocutiondetailModel.name;
    }else{
        self.title = @"聊天";
    }
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if(self.showType == MessageShowTypeClose){
        [QWCLICKEVENT qwTrackPageEnd:@"XPChatViewController_ygb"];
    }else if (self.showType == MessageShowTypeAsking) {
        [QWCLICKEVENT qwTrackPageEnd:@"XPChatViewController_dqd"];
    }else if (self.showType == MessageShowTypeAnswer) {
        [QWCLICKEVENT qwTrackPageEnd:@"XPChatViewController_jdz"];
    }
...
- (void)messageCenterInit{
    if (msgCenter == nil){
        ...
        if (!self.branchId) {
            self.branchId = self.interlocutiondetailModel.consultId;
        }
        msgCenter=[[ExpertXPMessageCenter alloc]initWithID:self.branchId type:self.chatType];
    ...
    [msgCenter setPharConsultBlock:^(InterlocutionDetailPageModel* model){
        //1：待解答，2：已回复，3：已关闭，4:抢而未答，5:已过期
        if (model.status == 3 || model.status == 5) {
            ...
            weakSelf.timeoutBottomView = [weakSelf setupTimeoutBottomView:model.status];
            ...
            weakSelf.animateButton.hidden = YES;
            weakSelf.mustAnswer = NO;
        }else{
            if(model.consultNotice.length > 0)
                [weakSelf setCountDownLabelText:model.consultNotice];
        }
    }];
}
    ...
    switch (messageType) {
        case MessageMediaTypeText:   //发送纯文本
        {
            [msgCenter sendMessage:messageModel success:^(id successObj) {
                if(self.mustAnswer) {
                    self.mustAnswer = NO;
                    [self setCountDownLabelText:self.countDownLabel.text];
                }
        ...
        case MessageMediaTypeCoupon://优惠活动 yqy
    ...
    if(actionSheet.tag == 1002)
    {
        ...
        if(buttonIndex == 1) {
            ...
            marketViewController.SendActivity = ^(QueryActivityInfo *model) {
                [self performSelector:@selector(toSendNormalActivity:) withObject:model afterDelay:.35];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(model.messageDeliveryType == MessageTypeSending) {
        ...
        [cell.headImageView setImageWithURL:[NSURL URLWithString:QWGLOBALMANAGER.configure.expertAvatarUrl] placeholderImage:[UIImage imageNamed:@"药店默认头像"]];
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
    ...
    //yqy 优惠活动 PMT
        ...
        vc.packPromotionId=model.richBody;
        vc.titlela=model.title;
        ...
        //        couponDeatilViewController.commonPromotionId = model.richBody;
    ...
    else if ([eventName isEqualToString:kResendButtonTapEventName])
    {
        ...
        alertView.tag = ExpertXPalertResendIdentifier;
        [alertView show];
    }else if ([eventName isEqualToString:kDeleteBtnTapEventName])
    {
        ...
        alertView.tag = ExpertXPalertDeleteIdentifier;
        [alertView show];
        ...
        // 头像点击
        MessageModel *model=[userInfo objectForKey:KMESSAGEKEY];
        if(model.messageDeliveryType == MessageTypeReceiving && !self.mustAnswer) {
            CustomerDetailInfoModelR *modelInfoR = [CustomerDetailInfoModelR new];
            modelInfoR.token = QWGLOBALMANAGER.configure.userToken;
            
//            modelInfoR.customer = self.interlocutiondetailModel.customerPassport;
...
- (UIView *)setupClosedBottomView:(CGRect)inputFrame
{
    ...
    closeLabel.font = fontSystem(kFontS1);
    closeLabel.textColor = RGBHex(qwColor6);
    closeLabel.frame = CGRectMake((APP_W-190)/2, 4.5, 190, 36);
...
{
    if(self.headerHintView) {
        return self.headerHintView;
    }
    UIView *headerView =[[UIView alloc] initWithFrame:CGRectMake(0, 0, APP_W, 32)];
    [headerView setBackgroundColor:RGBHex(qwColor16)];
    headerView.layer.borderWidth = 0.5;
    headerView.layer.borderColor = RGBHex(qwColor15).CGColor;
    headerView.alpha = 1.0;
    self.countDownLabel = [[UILabel alloc] init];
    _countDownLabel.font = fontSystem(kFontS4);
    _countDownLabel.textColor = RGBHex(qwColor12);
    ...
    [self setCountDownLabelText:@"该问题9分钟后过期"];
    return headerView;
}
...
- (void)setCountDownLabelText:(NSString *)text
{
    ...
    if(!self.mustAnswer && self.showType == MessageShowTypeAnswer) {
        ...
        CGFloat width = [text sizeWithFont:fontSystem(kFontS4)].width;
...
- (void)racingQuestion:(id)sender
{
    ...
    setting[@"consultId"] = self.interlocutiondetailModel.consultId;
...
- (void)layoutDifferentMessageType
{
    ...
    if(self.showType == MessageShowTypeAsking)
    {
        ...
        [QWCLICKEVENT qwTrackPageBegin:@"XPChatViewController_dqd"];
        ...
        rect.size.height = self.view.frame.size.height - 64 - inputFrame.size.height;
        self.tableMain.frame = rect;
        [QWCLICKEVENT qwTrackPageBegin:@"XPChatViewController_ygb"];
    }else{
        ...
        [QWCLICKEVENT qwTrackPageBegin:@"XPChatViewController_jdz"];
...
- (XHMessageInputView *)setupMessageInputView:(CGRect)inputFrame
{
    ...
    inputView.allowsSendVoice = ExpertXPallowsSendVoice;
...
- (void)setUpSharedMenuItem
{
    ...
    NSArray *plugIcons = @[@"photo_image",@"take_photo_image",@"im_icon_assistant.png"];
    NSArray *plugTitle = @[@"图片",@"拍照", @"医药助手"];
                ...
                MessageModel *mm = [[MessageModel alloc] initWithSpecialOffers:model.title
                                                                   ...
                                                                   activityUrl:model.imgUrl
                                                                    activityId:model.packPromotionId
                                                                       groupId:QWGLOBALMANAGER.configure.groupId
                                                                    branchLogo:QWGLOBALMANAGER.configure.expertAvatarUrl
                                                                        sender:self.messageSender timestamp:[NSDate date]
                                                                          UUID:[XMPPStream generateUUID]];
                [self sendMessage:mm messageBodyType:mm.messageMediaType];
...
{
    if (alertView.tag == ExpertXPalertResendIdentifier) {
        if (buttonIndex == 0) {
    ...
    } else if (alertView.tag == ExpertXPalertDeleteIdentifier) {
        if (buttonIndex == 1) {
            // 删除
        ...
        if(buttonIndex == 1) {
            ...
            NSMutableDictionary *setting = [NSMutableDictionary dictionary];
            setting[@"token"] = StrFromObj(QWGLOBALMANAGER.configure.expertToken);
            setting[@"consultId"] = StrFromObj(self.interlocutiondetailModel.consultId);
            [UserChat ImConsultIgnoreWithParams:setting success:^(id obj) {
                BaseAPIModel *model = [BaseAPIModel parse:obj];
                if([model.apiStatus integerValue] == 0) {
                    [QWGLOBALMANAGER postNotif:NotiRefreshExpertAllConsult data:nil object:nil];
                }else{
                    [SVProgressHUD showErrorWithStatus:model.apiMessage duration:0.8f];
                }
...
- (IBAction)popVCAction:(id)sender{
    ...
    if(self.mustAnswer) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"确定要放弃回答此条问题吗？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
        alertView.tag = 1114;
        [alertView show];
    }else{
...
- (UIView *)setupTimeoutBottomView:(NSUInteger)type
{
    CGRect inputFrame = CGRectMake(0.0f,
    ...
    closeLabel.textAlignment = NSTextAlignmentCenter;
    [bottomView addSubview:closeLabel];
    [bottomView bringSubviewToFront:closeLabel];
    return bottomView;
}
...
- (UIView *)setupRacingBottomView:(CGRect)inputFrame
{
    ...
    racingButton.frame = CGRectMake(65, 4.5, APP_W-130, 36);
