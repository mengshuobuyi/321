...
{    
    EditCityExpressViewController *vc = [[UIStoryboard storyboardWithName:@"StoreInfo" bundle:nil] instantiateViewControllerWithIdentifier:@"EditCityExpressViewController"];
    vc.deliverTimeText = self.shippingMethodModel.expressTime;
    vc.quickPriceText = [NSString stringWithFormat:@"%.0f",self.shippingMethodModel.expressFee];
    vc.freePriceText = [NSString stringWithFormat:@"%.0f",self.shippingMethodModel.expressFreeFee];
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
