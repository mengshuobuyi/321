...
/*
    ...
    h5/imConsult/queryQADetail     全量问答详情
    h5/imConsult/loopNews"         增量问答详情
    h5/imConsult/reply"            问答回复发消息
 */
...
@implementation ExpertXPMessageCenter
...
- (void)sendPhotosMessage:(MessageModel* )model success:(IMSuccessBlock)success failure:(IMFailureBlock)failure  uploadProgressBlock:(void (^)(MessageModel* target, float progress ))uploadProgressBlock
{
    ...
    params[@"token"] = StrFromObj(QWGLOBALMANAGER.configure.expertToken);
...
- (void)getAllMessages
{
    ...
    setting[@"consultId"] = StrFromObj(self.oID);
    [UserChat ImConsultQueryQADetailWithParams:setting success:^(id obj) {
        InterlocutionDetailPageModel *page = [InterlocutionDetailPageModel parse:obj Elements:[InterlocutionDetailListModel class] forAttribute:@"qaDetailListVOs"];
        [self checkMessagesFromAPI:page type:IMListAll];
        [self setOffset:0];
        [self showCurrentMessages:IMListAll successData:page];
...
- (void)pollMessages
{
    ...
    setting[@"token"] = QWGLOBALMANAGER.configure.expertToken;
    setting[@"consultId"] = StrFromObj(self.oID);
    [UserChat ImConsultLoopNewsWithParams:setting success:^(id obj) {
        InterlocutionDetailPageModel *page = [InterlocutionDetailPageModel parse:obj Elements:[InterlocutionDetailListModel class] forAttribute:@"qaDetailListVOs"];
        if ([self isClose]) {
            return ;
        }
        [self checkNoDataTimes:page.qaDetailListVOs.count];
        [self checkMessagesFromAPI:page type:IMListPolling];
        if (page.qaDetailListVOs.count)
            [self showCurrentMessages:IMListPolling successData:page];
    } failure:^(HttpException *e) {
...
- (void)sendAMessage:(MessageModel*)model success:(IMSuccessBlock)success failure:(IMFailureBlock)failure
{
    ...
    InterlocutionXPCreate *mm = [self buildXPCreateFromMessage:model];
    
    ++self.sendingCount;
    
    [UserChat ImConsultReplyWithParams:mm success:^(id obj) {
        self.sendingCount=self.sendingCount-1;
        if([obj[@"apiStatus"] integerValue] == 0) {
            
            qwmsg.UUID=StrFromObj(obj[@"detailId"]);
...
#pragma mark ---- 发送消息 MessageModel 转 入参model ----
- (InterlocutionXPCreate*)buildXPCreateFromMessage:(MessageModel *)model
{
    InterlocutionXPCreate *mm = [InterlocutionXPCreate new];
    mm.token = QWGLOBALMANAGER.configure.expertToken;
...
- (BOOL)checkMessagesFromAPI:(id)mode type:(IMListType)lType
{
    ...
    if([mode isKindOfClass:[CustomerConsultDetailList class]]){
        ...
        arrDetails=model.details;
        num=model.details.count;
    }else if ([mode isKindOfClass:[InterlocutionDetailPageModel class]]){
        InterlocutionDetailPageModel *model=mode;
        arrDetails=model.qaDetailListVOs;
        num=model.qaDetailListVOs.count;
    }
    else return NO;
    ...
    for(InterlocutionDetailListModel *detail in arrDetails)
    {
...
- (QWXPMessage *)buildQWMessageFromAPIModel:(id)mode detail:(InterlocutionDetailListModel*)detail{
    ...
    if(detail.myselfFlag) {
        ...
        qwmsg.avatorUrl = StrFromObj(detail.headImg);
    }
