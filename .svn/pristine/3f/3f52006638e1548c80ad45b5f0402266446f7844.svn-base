...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, APP_W, 40);
        
        _menuColor = RGBHex(qwColor7);//按钮初始化的颜色
        _array = array;

        _numOfMenu = _array.count;//选择器的数组
        
        CGFloat textLayerInterval = self.frame.size.width / ( _numOfMenu * 2);
        CGFloat separatorLineInterval = self.frame.size.width / _numOfMenu;//分割线
        
        _titles = [[NSMutableArray alloc] initWithCapacity:_numOfMenu];//标题
        _indicators = [[NSMutableArray alloc] initWithCapacity:_numOfMenu];//标题后的箭头
        ...
        for (int i = 0; i < _numOfMenu; i++) {
            ...
            CAShapeLayer *indicator = [self creatIndicatorWithColor:_menuColor andPosition:CGPointMake(position.x + title.bounds.size.width / 2 + 12, self.frame.size.height / 2)];
            ...
            if (i != _numOfMenu - 1) {
                ...
                CAShapeLayer *separator = [self creatSeparatorLineWithColor:_menuColor andPosition:separatorPosition];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        UIView *line3 = [[UIView alloc]initWithFrame:CGRectMake(0,0,APP_W, 0.5)];
