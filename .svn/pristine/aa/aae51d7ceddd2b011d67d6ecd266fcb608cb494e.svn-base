...
QWJSBridge.resources = {base: true};
...
QWJSBridge.Channel.prototype.subscribe = function(f, c, g) {
    ...
    var func = f;
    if (typeof c === "object" && typeof f === "function") { func = QWJSBridge.close(c, f); }
...
QWJSBridge.Channel.prototype.subscribeOnce = function(f, c) {
    ...
    if (this.fired) {
        if (typeof c === "object" && typeof f === "function") { f = QWJSBridge.close(c, f); }
...
if (typeof _nativeReady !== 'undefined') { QWJSBridge.onNativeReady.fire(); }
...
document.addEventListener = function(evt, handler, capture) {
    ...
    if (e === 'deviceready') {
        QWJSBridge.onDeviceReady.subscribeOnce(handler);
    }
    else {
        // If subscribing to Android backbutton
        if (e === 'backbutton') {
            QWJSBridge.exec(null, null, "App", "overrideBackbutton", [true]);
        }
        
        // If subscribing to an event that is handled by a plugin
        else if (typeof QWJSBridge.documentEventHandler[e] !== "undefined") {
            if (QWJSBridge.documentEventHandler[e](e, handler, true)) {
                return; // Stop default behavior
            }
        }
        
        QWJSBridge.m_document_addEventListener.call(document, evt, handler, capture);
    }
};
...
window.addEventListener = function(evt, handler, capture) {
    ...
    // If subscribing to an event that is handled by a plugin
    if (typeof QWJSBridge.windowEventHandler[e] !== "undefined") {
        if (QWJSBridge.windowEventHandler[e](e, handler, true)) {
            return; // Stop default behavior
        }
    }
        
    QWJSBridge.m_window_addEventListener.call(window, evt, handler, capture);
};

// Intercept calls to document.removeEventListener and watch for events that
// are generated by QWJSBridge native code
QWJSBridge.m_document_removeEventListener = document.removeEventListener;

// Intercept calls to window.removeEventListener
QWJSBridge.m_window_removeEventListener = window.removeEventListener;
...
document.removeEventListener = function(evt, handler, capture) {
    ...
    if (e === 'backbutton') {
        QWJSBridge.exec(null, null, "App", "overrideBackbutton", [false]);
    }

    // If unsubcribing from an event that is handled by a plugin
    if (typeof QWJSBridge.documentEventHandler[e] !== "undefined") {
        if (QWJSBridge.documentEventHandler[e](e, handler, false)) {
            return; // Stop default behavior
        }
    }

    QWJSBridge.m_document_removeEventListener.call(document, evt, handler, capture);
};
...
window.removeEventListener = function(evt, handler, capture) {
    ...
    // If unsubcribing from an event that is handled by a plugin
    if (typeof QWJSBridge.windowEventHandler[e] !== "undefined") {
        if (QWJSBridge.windowEventHandler[e](e, handler, false)) {
            return; // Stop default behavior
        }
    }

    QWJSBridge.m_window_removeEventListener.call(window, evt, handler, capture);
};
 ...
 */
QWJSBridge.fireDocumentEvent = function(type, data) {
 ...
 * @param {Object} data             Data to send with event
 */
QWJSBridge.fireWindowEvent = function(type, data) {
    var e = document.createEvent('Events');
    e.initEvent(type);
    if (data) {
        for (var i in data) {
...
QWJSBridge.clone = function(obj) {
    ...
    if(obj instanceof Array){
        ...
        for(i = 0; i < obj.length; ++i){
            retVal.push(QWJSBridge.clone(obj[i]));
        }
        return retVal;
    }
    ...
    for(i in obj){
        if(!(i in retVal) || retVal[i] !== obj[i]) {
            retVal[i] = QWJSBridge.clone(obj[i]);
        }
    }
    return retVal;
};
 ...
 */
QWJSBridge.exec = function(success, fail, service, action, args) {
    try {
        var callbackId = service + QWJSBridge.callbackId++;
        if (success || fail) {
            QWJSBridge.callbacks[callbackId] = {success:success, fail:fail};
        }
 ...
 * @return {String}
 */
QWJSBridge.createUUID = function() {
    return QWJSBridge.UUIDcreatePart(4) + '-' +
        QWJSBridge.UUIDcreatePart(2) + '-' +
        QWJSBridge.UUIDcreatePart(2) + '-' +
        QWJSBridge.UUIDcreatePart(2) + '-' +
        QWJSBridge.UUIDcreatePart(6);
};

QWJSBridge.UUIDcreatePart = function(length) {
    var uuidpart = "";
    var i, uuidchar;
    for (i=0; i<length; i++) {
...
QWJSBridge.close = function(context, func, params) {
 ...
 */
QWJSBridge.includeJavascript = function(jsfile, successCallback) {
...
if (!QWJSBridge.hasResource("accelerometer")) {
QWJSBridge.addResource("accelerometer");
...
Accelerometer.prototype.getCurrentAcceleration = function(successCallback, errorCallback, options) {
    ...
    // Get acceleration
    QWJSBridge.exec(successCallback, errorCallback, "Accelerometer", "getAcceleration", []);
};
    ...
    QWJSBridge.exec(
        function(timeout) {
            if (timeout < (frequency + 10000)) {
                QWJSBridge.exec(null, null, "Accelerometer", "setTimeout", [frequency + 10000]);
            }
        },
        function(e) { }, "Accelerometer", "getTimeout", []);

    // Start watch timer
    var id = QWJSBridge.createUUID();
...
if (!QWJSBridge.hasResource("app")) {
QWJSBridge.addResource("app");
(function() {
...
App.prototype.clearCache = function() {
    QWJSBridge.exec(null, null, "App", "clearCache", []);
};
...
App.prototype.loadUrl = function(url, props) {
    QWJSBridge.exec(null, null, "App", "loadUrl", [url, props]);
};
...
App.prototype.cancelLoadUrl = function() {
    QWJSBridge.exec(null, null, "App", "cancelLoadUrl", []);
};
...
App.prototype.clearHistory = function() {
    QWJSBridge.exec(null, null, "App", "clearHistory", []);
};
...
App.prototype.backHistory = function() {
    QWJSBridge.exec(null, null, "App", "backHistory", []);
};
...
App.prototype.overrideBackbutton = function(override) {
    QWJSBridge.exec(null, null, "App", "overrideBackbutton", [override]);
};
...
App.prototype.exitApp = function() {
	return QWJSBridge.exec(null, null, "App", "exitApp", []);
};

/**
 * Add entry to approved list of URLs (whitelist) that will be loaded into QWJSBridge container instead of default browser.
 * 
 * @param origin		URL regular expression to allow
 * @param subdomains	T=include all subdomains under origin
 */
App.prototype.addWhiteListEntry = function(origin, subdomains) {
	return QWJSBridge.exec(null, null, "App", "addWhiteListEntry", [origin, subdomains]);	
};
...
if (!QWJSBridge.hasResource("battery")) {
QWJSBridge.addResource("battery");
...
Battery.prototype.eventHandler = function(eventType, handler, add) {
    ...
    if (add) {
        ...
        if (me._batteryListener.length === 0 && me._lowListener.length === 0 && me._criticalListener.length === 0) {
            QWJSBridge.exec(me._status, me._error, "Battery", "start", []);
        }
        ...
        if (me._batteryListener.length === 0 && me._lowListener.length === 0 && me._criticalListener.length === 0) {
            QWJSBridge.exec(null, null, "Battery", "stop", []);
        }
    }
};
...
Battery.prototype._status = function(info) {
	if (info) {
		...
		if (me._level != info.level || me._isPlugged != info.isPlugged) {
			...
			QWJSBridge.fireWindowEvent("batterystatus", info);

			// Fire low battery event
			if (info.level == 20 || info.level == 5) {
				if (info.level == 20) {
					QWJSBridge.fireWindowEvent("batterylow", info);
				}
				else {
					QWJSBridge.fireWindowEvent("batterycritical", info);
				}
			}
		}
...
QWJSBridge.addConstructor(function() {
    if (typeof navigator.battery === "undefined") {
        ...
        QWJSBridge.addWindowEventHandler("batterystatus", navigator.battery.eventHandler);
        QWJSBridge.addWindowEventHandler("batterylow", navigator.battery.eventHandler);
        QWJSBridge.addWindowEventHandler("batterycritical", navigator.battery.eventHandler);
    }
});
}
/*
...
if (!QWJSBridge.hasResource("camera")) {
QWJSBridge.addResource("camera");
...
Camera.prototype.getPicture = function(successCallback, errorCallback, options) {
    ...
    QWJSBridge.exec(successCallback, errorCallback, "Camera", "takePicture", [options]);
};
...
if (!QWJSBridge.hasResource("capture")) {
QWJSBridge.addResource("capture");
...
MediaFile.prototype.getFormatData = function(successCallback, errorCallback){
	QWJSBridge.exec(successCallback, errorCallback, "Capture", "getFormatData", [this.fullPath, this.type]);
};
...
Capture.prototype.captureAudio = function(successCallback, errorCallback, options){
	QWJSBridge.exec(successCallback, errorCallback, "Capture", "captureAudio", [options]);
};
...
Capture.prototype.captureImage = function(successCallback, errorCallback, options){
	QWJSBridge.exec(successCallback, errorCallback, "Capture", "captureImage", [options]);
};
...
Capture.prototype.captureVideo = function(successCallback, errorCallback, options){
	QWJSBridge.exec(successCallback, errorCallback, "Capture", "captureVideo", [options]);
};
...
if (!QWJSBridge.hasResource("compass")) {
QWJSBridge.addResource("compass");
...
Compass.prototype.getCurrentHeading = function(successCallback, errorCallback, options) {
    ...
    // Get heading
    QWJSBridge.exec(successCallback, errorCallback, "Compass", "getHeading", []);
};
...
Compass.prototype.watchHeading= function(successCallback, errorCallback, options) {
    ...
    QWJSBridge.exec(
        function(timeout) {
            if (timeout < (frequency + 10000)) {
                QWJSBridge.exec(null, null, "Compass", "setTimeout", [frequency + 10000]);
            }
        },
...
if (!QWJSBridge.hasResource("contact")) {
QWJSBridge.addResource("contact");
...
Contact.prototype.remove = function(successCB, errorCB) {
    if (this.id === null) {
        ...
        QWJSBridge.exec(successCB, errorCB, "Contacts", "remove", [this.id]);
    }
};
...
Contact.prototype.clone = function() {
    var clonedContact = QWJSBridge.clone(this);
    var i;
...
Contact.prototype.save = function(successCB, errorCB) {
    QWJSBridge.exec(successCB, errorCB, "Contacts", "save", [this]);
};
...
Contacts.prototype.find = function(fields, successCB, errorCB, options) {
    ...
    if (fields === null || fields === "undefined" || fields.length === "undefined" || fields.length <= 0) {
        ...
        QWJSBridge.exec(successCB, errorCB, "Contacts", "search", [fields, options]);        
    }
};
...
if (!QWJSBridge.hasResource("crypto")) {
QWJSBridge.addResource("crypto");
...
Crypto.prototype.encrypt = function(seed, string, callback) {
    ...
    QWJSBridge.exec(null, null, "Crypto", "encrypt", [seed, string]);
};

Crypto.prototype.decrypt = function(seed, string, callback) {
    this.decryptWin = callback;
    QWJSBridge.exec(null, null, "Crypto", "decrypt", [seed, string]);
};
...
if (!QWJSBridge.hasResource("device")) {
QWJSBridge.addResource("device");
 ...
 */
var Device = function() {
    this.available = QWJSBridge.available;
    this.platform = null;
    this.version = null;
    this.name = null;
    this.uuid = null;
    this.QWJSBridge = null;
    ...
    this.getInfo(
        function(info) {
            ...
            me.name = info.name;
            me.uuid = info.uuid;
            me.QWJSBridge = info.QWJSBridge;
            QWJSBridge.onQWJSBridgeInfoReady.fire();
        },
        function(e) {
            me.available = false;
            console.log("Error initializing QWJSBridge: " + e);
            alert("Error initializing QWJSBridge: "+e);
        });
        
        function successCallbackd()
        {
        alert("124llllljjjjjjj");
        }
        
         QWJSBridge.exec(successCallbackd, errorCallback, "Device", "getDeviceInfo", []);
};
...
Device.prototype.getInfo = function(successCallback, errorCallback) {
    ...
    if (errorCallback && (typeof errorCallback !== "function")) {
     ...
     alert("test  decview  faile ");
        console.log("Device Error: errorCallback is not a function");
        return;
    }

    // Get info
    QWJSBridge.exec(successCallback, errorCallback, "Device", "getDeviceInfo", []);
};
...
if (!QWJSBridge.hasResource("file")) {
QWJSBridge.addResource("file");
...
DirectoryReader.prototype.readEntries = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "readEntries", [this.fullPath]);
};
...
DirectoryEntry.prototype.copyTo = function(parent, newName, successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "copyTo", [this.fullPath, parent, newName]);
};
...
DirectoryEntry.prototype.getMetadata = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "getMetadata", [this.fullPath]);
};
...
DirectoryEntry.prototype.getParent = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "getParent", [this.fullPath]);
};
...
DirectoryEntry.prototype.moveTo = function(parent, newName, successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "moveTo", [this.fullPath, parent, newName]);
};
...
DirectoryEntry.prototype.remove = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "remove", [this.fullPath]);
};
...
DirectoryEntry.prototype.getDirectory = function(path, options, successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "getDirectory", [this.fullPath, path, options]);
};
...
DirectoryEntry.prototype.getFile = function(path, options, successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "getFile", [this.fullPath, path, options]);
};
...
DirectoryEntry.prototype.removeRecursively = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "removeRecursively", [this.fullPath]);
};
...
FileEntry.prototype.file = function(successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "getFileMetadata", [this.fullPath]);
};
...
LocalFileSystem.prototype.requestFileSystem = function(type, size, successCallback, errorCallback) {
    if (type < 0 || type > 3) {
        ...
        QWJSBridge.exec(successCallback, errorCallback, "File", "requestFileSystem", [type, size]);
    }
};
...
LocalFileSystem.prototype.resolveLocalFileSystemURI = function(uri, successCallback, errorCallback) {
    QWJSBridge.exec(successCallback, errorCallback, "File", "resolveLocalFileSystemURI", [uri]);
};
...
if (!QWJSBridge.hasResource("filetransfer")) {
QWJSBridge.addResource("filetransfer");
...
FileTransfer.prototype.upload = function(filePath, server, successCallback, errorCallback, options, debug) {
    ...
    QWJSBridge.exec(successCallback, errorCallback, 'FileTransfer', 'upload', [filePath, server, fileKey, fileName, mimeType, params, debug, chunkedMode]);
};
...
if (!QWJSBridge.hasResource("geolocation")) {
QWJSBridge.addResource("geolocation");
...
Geolocation.prototype.getCurrentPosition = function(successCallback, errorCallback, options) {
    ...
    QWJSBridge.exec(null, null, "Geolocation", "getCurrentLocation", [enableHighAccuracy, timeout, maximumAge]);
};
...
Geolocation.prototype.watchPosition = function(successCallback, errorCallback, options) {
    ...
    QWJSBridge.exec(null, null, "Geolocation", "start", [id, enableHighAccuracy, timeout, maximumAge]);
    return id;
};
...
Geolocation.prototype.clearWatch = function(id) {
    QWJSBridge.exec(null, null, "Geolocation", "stop", [id]);
    delete navigator._geo.listeners[id];
};

/**
 * Force the QWJSBridge geolocation to be used instead of built-in.
 */
Geolocation.usingQWJSBridge = false;
Geolocation.useQWJSBridge = function() {
    if (Geolocation.usingQWJSBridge) {
        return;
    }
    Geolocation.usingQWJSBridge = true;
...
QWJSBridge.addConstructor(function() {
    ...
    // No native geolocation object for Android 1.x, so use QWJSBridge geolocation
...
if (!QWJSBridge.hasResource("media")) {
QWJSBridge.addResource("media");
...
var Media = function(src, successCallback, errorCallback, statusCallback, positionCallback) {
    ...
    this.id = QWJSBridge.createUUID();
    QWJSBridge.mediaObjects[this.id] = this;
    this.src = src;
...
Media.prototype.play = function() {
    QWJSBridge.exec(null, null, "Media", "startPlayingAudio", [this.id, this.src]);
};
...
Media.prototype.stop = function() {
    return QWJSBridge.exec(null, null, "Media", "stopPlayingAudio", [this.id]);
};
...
Media.prototype.seekTo = function(milliseconds) {
    QWJSBridge.exec(null, null, "Media", "seekToAudio", [this.id, milliseconds]);
};
...
Media.prototype.pause = function() {
    QWJSBridge.exec(null, null, "Media", "pausePlayingAudio", [this.id]);
};
...
Media.prototype.getCurrentPosition = function(success, fail) {
    QWJSBridge.exec(success, fail, "Media", "getCurrentPositionAudio", [this.id]);
};
...
Media.prototype.startRecord = function() {
    QWJSBridge.exec(null, null, "Media", "startRecordingAudio", [this.id, this.src]);
};
...
Media.prototype.stopRecord = function() {
    QWJSBridge.exec(null, null, "Media", "stopRecordingAudio", [this.id]);
};
...
Media.prototype.release = function() {
    QWJSBridge.exec(null, null, "Media", "release", [this.id]);
};
...
Media.prototype.setVolume = function(volume) {
    QWJSBridge.exec(null, null, "Media", "setVolume", [this.id, volume]);
};
 ...
 */
QWJSBridge.Media.getMediaObject = function(id) {
    return QWJSBridge.mediaObjects[id];
};
 ...
 */
QWJSBridge.Media.onStatus = function(id, msg, value) {
    var media = QWJSBridge.mediaObjects[id];
    // If state update
...
if (!QWJSBridge.hasResource("network")) {
QWJSBridge.addResource("network");
        ...
        function(type) {
            ...
            if (type == "none") {
                ...
                me._timer = setTimeout(function(){
                    me.type = type;
                    QWJSBridge.fireDocumentEvent('offline');
                    me._timer = null;
                    }, me.timeout);
            } else {
                ...
                me.type = type;
                QWJSBridge.fireDocumentEvent('online');
            }
            
            // should only fire this once
            if (me._firstRun) {
                me._firstRun = false;
                QWJSBridge.onQWJSBridgeConnectionReady.fire();
            }            
        },
        function(e) {
            // If we can't get the network info we should still tell QWJSBridge
...
Connection.prototype.getInfo = function(successCallback, errorCallback) {
    // Get info
    QWJSBridge.exec(successCallback, errorCallback, "Network Status", "getConnectionInfo", []);
};
...
if (!QWJSBridge.hasResource("notification")) {
QWJSBridge.addResource("notification");
...
Notification.prototype.alert = function(message, completeCallback, title, buttonLabel) {
    ...
    QWJSBridge.exec(completeCallback, null, "Notification", "alert", [message,_title,_buttonLabel]);
};
...
Notification.prototype.confirm = function(message, resultCallback, title, buttonLabels) {
    ...
    QWJSBridge.exec(resultCallback, null, "Notification", "confirm", [message,_title,_buttonLabels]);
};
...
Notification.prototype.activityStart = function() {
    QWJSBridge.exec(null, null, "Notification", "activityStart", ["Busy","Please wait..."]);
};
...
Notification.prototype.activityStop = function() {
    QWJSBridge.exec(null, null, "Notification", "activityStop", []);
};
...
Notification.prototype.progressStart = function(title, message) {
    QWJSBridge.exec(null, null, "Notification", "progressStart", [title, message]);
};
...
Notification.prototype.progressValue = function(value) {
    QWJSBridge.exec(null, null, "Notification", "progressValue", [value]);
};
...
Notification.prototype.progressStop = function() {
    QWJSBridge.exec(null, null, "Notification", "progressStop", []);
};
...
Notification.prototype.vibrate = function(mills) {
    QWJSBridge.exec(null, null, "Notification", "vibrate", [mills]);
};
...
Notification.prototype.beep = function(count) {
    QWJSBridge.exec(null, null, "Notification", "beep", [count]);
};
...
if (!QWJSBridge.hasResource("position")) {
QWJSBridge.addResource("position");
...
if (!QWJSBridge.hasResource("storage")) {
QWJSBridge.addResource("storage");
...
DroidDB_Tx.prototype.executeSql = function(sql, params, successCallback, errorCallback) {
    ...
    // Call native code
    QWJSBridge.exec(null, null, "Storage", "executeSql", [sql, params, query.id]);
};
...
var DroidDB_openDatabase = function(name, version, display_name, size) {
    QWJSBridge.exec(null, null, "Storage", "openDatabase", [name, version, display_name, size]);
      ...
      this.db.transaction(
        function (transaction) {
          ...
          transaction.executeSql('SELECT * FROM storage', [], function(tx, result) {
            ...
            QWJSBridge.initializationComplete("cupcakeStorage");
          });
...
QWJSBridge.addConstructor(function() {
    ...
    if (typeof window.localStorage === "undefined") {
        ...
        QWJSBridge.waitForInitialization("cupcakeStorage");
    }
});
}
