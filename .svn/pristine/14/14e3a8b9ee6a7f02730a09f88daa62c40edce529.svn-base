...
+ (void)playAlarmAudio
{
    if([QWUserDefault getBoolBy:APP_Alarm_SOUND_ENABLE]){
        NSString *path = [[NSBundle mainBundle] pathForResource:@"闹钟提醒音乐" ofType:@"mp3"];
        [[XHAudioPlayerHelper shareInstance] playAudioWithFileName:path];
    }
    if([QWUserDefault getBoolBy:APP_Alarm_VIBRATION_ENABLE])
        [[XHAudioPlayerHelper shareInstance] setupVibrateTimer];
}

+ (void)stopAlarmAudio
{
    [[XHAudioPlayerHelper shareInstance] stopAudio];
    [[XHAudioPlayerHelper shareInstance] releaseVibrateTimer];
}

- (void)releaseVibrateTimer
{
    [vibrateTimer invalidate];
    vibrateTimer = nil;
}

- (void)setupVibrateTimer
{
    [vibrateTimer invalidate];
    vibrateTimer = nil;
    vibrateTimer = [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(playVibrate) userInfo:nil repeats:YES];
}
...
+ (void)playMessageReceivedSound
{
    ...
    if([QWUserDefault getBoolBy:APP_SOUND_ENABLE]){
        [XHAudioPlayerHelper playSoundWithName:@"messageReceived" type:@"aiff"];
    }
    if([QWUserDefault getBoolBy:APP_VIBRATION_ENABLE])
    {
        AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
    }
}
...
+ (void)playMessageSentSound
{
    ...
    if([QWUserDefault getBoolBy:APP_SOUND_ENABLE])
        [XHAudioPlayerHelper playSoundWithName:@"messageSent" type:@"aiff"];
}
