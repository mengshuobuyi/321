...
#import "QWPopupView_circle.h"
#import <QuartzCore/QuartzCore.h>
/////////////////////////////////////////////////////////////////////////////////////////////////////////
@interface QWPopupView_circle ()
...
@end
/////////////////////////////////////////////////////////////////////////////////////////////////////////
@implementation QWPopupView_circle
...
-(void)initImgBtnItems
{
    ...
    if ([itemsArray count] < 4) {//加载第一张背景图
        bgImg = [UIImage imageNamed:@"QW_circleBgimage1"];
    }
    else if ([itemsArray count] < 5) {//加载第二张背景图
        bgImg = [UIImage imageNamed:@"QW_circleBgimage2"];
    }
    else if ([itemsArray count] < 6) {//加载第三张背景图
        bgImg = [UIImage imageNamed:@"QW_circleBgimage3"];
    }
    else if ([itemsArray count] < MAXCOUNT) {//加载第四张背景图
        bgImg = [UIImage imageNamed:@"QW_circleBgimage4"];
    }
                   ...
                   isClockWise:(bool)isclockWise
{
    ...
    QWPopupView_circleItem * btn = [[QWPopupView_circleItem alloc ]initWithFrame:CGRectMake(0, 0, img.size.width, img.size.height)];
    btn.delegate = self;
...
//功能键点击
-(void)CircleItemClick:(QWPopupView_circleItem *)btn
{
    if (btn.tag == -1) {//隐藏功能键
        [self hideItems];
        btn.tag = 0;
...
-(void)showItems
{
    ...
    for (int i = 0 ; i < [itemsArray count]; i++) {
        QWPopupView_circleItem * btn = [itemsArray objectAtIndex:i];
...
#pragma mark -
#pragma mark QWPopupView_circleItemDelegate
