...
@implementation NewQCSlideSwitchView
{
    CGFloat kHeightOfTopScrollView;
    CGFloat kWidthOfButtonMargin;
    CGFloat kWidthOfAdjustHealthIndicator;
    CGFloat kFontSizeOfTabButton;
    NSUInteger kTagOfRightSideButton;
    BOOL need6PFix;
}
#pragma mark - 初始化参数
...
- (void)initValues
{
    if (IS_IPHONE_6P && need6PFix) {
      kHeightOfTopScrollView = 54.0f;
      kWidthOfButtonMargin = 5.0f;// 2.0f
    kWidthOfAdjustHealthIndicator = 5.0f;
      kFontSizeOfTabButton = 20.0f;
       kTagOfRightSideButton = 999;
    }
    else
    {
        kHeightOfTopScrollView = 39.0f;
        kWidthOfButtonMargin = 5.0f;// 2.0f
        kWidthOfAdjustHealthIndicator = 5.0f;
        kFontSizeOfTabButton = 14.0f;
        kTagOfRightSideButton = 999;
    }
    DebugLog(@"height is %f now",kHeightOfTopScrollView);
    //创建顶部可滑动的tab
...
-(id)initWithFrame:(CGRect)frame AndNeed6PFix:(BOOL)fix {
...
- (void)createNameButtons
{
    ...
    DebugLog(@"%f,%f",_shadowImage.size.width,kHeightOfTopScrollView);
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        messageLabel.font = fontSystem(kFontS6);
        ...
        if (i == 0) {
            if([_viewArray count] == 1)
                _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0,buttonWidth, kHeightOfTopScrollView);
            else
                _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0,buttonWidth, kHeightOfTopScrollView);
//            button.selected = YES;
...
//    }
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        if (i == 0) {
            if([_viewArray count] == 1)
                _shadowImageView.frame = CGRectMake(kWidthOfAdjustHealthIndicator, 0,textSize.width, kHeightOfTopScrollView);
            else
                _shadowImageView.frame = CGRectMake(kWidthOfAdjustHealthIndicator, 0,textSize.width, kHeightOfTopScrollView);
            //            button.selected = YES;
