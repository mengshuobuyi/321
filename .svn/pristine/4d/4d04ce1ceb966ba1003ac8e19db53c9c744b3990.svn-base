...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        greenLine.backgroundColor = RGBHex(qwColor14);
        ...
        button.layer.borderColor = RGBHex(qwColor14).CGColor;
...
- (void)setInfoDict:(NSDictionary *)infoDict
{
    ...
    CGSize syptomStrSize = getTempTextSize(sameStr, font, APP_W-20);
    
    DisesaeDetailInfoButton * btn = (DisesaeDetailInfoButton *)[self viewWithTag:kButtonTag];
    btn.buttonName = buttonTitle;
    [btn setTitleColor:RGBHex(qwColor14) forState:UIControlStateNormal];
    btn.titleLabel.font = font;
    [btn setFrame:CGRectMake(k0, relate_y, getTempTextSize(buttonTitle, font, APP_W-20).width + 20, kRelateButtonHeight)];
