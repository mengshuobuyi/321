...
//    }
    //点对点变群聊的，找得以前的代码---cj
    if(self.consultingModel && self.consultingModel.customerIndex)
    {
        self.title = self.consultingModel.customerIndex;
    }else{
        self.title = @"聊天";
    }
    ...
    NSString *groupName = @"";
//    if(!StrIsEmpty(self.consultInfo.storeModel.name)) {
    ...
    if(actionSheet.tag == 1002)
    {
        if(buttonIndex == 0) {
            ...
            quickSearchDrugViewController.returnValueBlock = ^(productclassBykwId *model){
                ...
                MessageModel *medicineModel = [[MessageModel alloc] initWithStoreMedicine:model.proName productId:model.proId imageUrl:model.imgUrl spec:model.spec branchId:@"" branchProId:@"" sender:self.messageSender timestamp:[NSDate date] UUID:[XMPPStream generateUUID]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(model.messageDeliveryType == MessageTypeSending) {
        ...
        cell.messageModel = model;
        [cell updateBubbleViewConsTraint:model];
        [cell.headImageView setImageWithURL:[NSURL URLWithString:self.consultingModel.customerAvatarUrl] placeholderImage:[UIImage imageNamed:@"expert_ic_people"]];
//        [cell.headImageView setImageWithURL:[NSURL URLWithString:model.avatorUrl] placeholderImage:[UIImage imageNamed:@"个人资料_头像"]];
...
- (UIView *)setupCountDownHeaderView
{
    ...
    [self setCountDownLabelText:@"该问题 分钟后过期"];
    return headerView;
}
...
//    NSArray *plugTitle = @[@"图片",@"拍照",@"发送药店地址",@"本店优惠", @"本店活动",@"常用话术", @"医药助手"];
    ...
    NSArray *plugIcons = @[@"photo_image",@"take_photo_image",@"药店地址",@"常用话术"];
    NSArray *plugTitle = @[@"图片",@"拍照",@"发送药店地址",@"常用话术"];
