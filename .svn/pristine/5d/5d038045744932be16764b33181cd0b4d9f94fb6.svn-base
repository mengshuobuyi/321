...
- (instancetype)initWithArr:(NSArray *)arr
{
    if (self = [super init]) {
        ...
        [boxView setBackgroundColor:RGBHex(qwColor16)];
        boxView.layer.borderColor = RGBHex(qwColor15).CGColor;
...
- (void)setInfoDict:(NSDictionary *)infoDict
{
    ...
    for (int i = 0; i < buttonArr.count; i++) {
        ...
        CGSize buttonSize = getTempTextSize(buttonName, buttonFont, APP_W-20);
